...
function HomePage() {
  ...
  function canAccess(href) {
    if (href === '/') return true
    if (isAdmin || isParticipant) return href !== '/admin' || isAdmin
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: ok ? '#fff' : th.textSec,
                      fontWeight: 700, fontSize: 14, border: ok ? 'none' : `1px solid ${th.border}`,
                      cursor: 'pointer', fontFamily: 'inherit',
                      boxShadow: ok ? `0 4px 14px ${th.shadow}` : 'none',
                      opacity: ok ? 1 : 0.6,
            ...
            {showLoginMsg && (
              <div style={{
                ...
                <span style={{ fontSize: 14 }}>
                  {isGoogleOnly
                    ? <><span style={{ color: '#FF4860', fontWeight: 600 }}>Сначала </span><Link href="/register" style={{ color: th.primary, fontWeight: 700, textDecoration: 'underline' }}>завершите регистрацию</Link></>
                    : <><span style={{ color: '#FF4860', fontWeight: 600 }}>Для доступа необходимо </span><Link href="/login" style={{ color: th.primary, fontWeight: 700, textDecoration: 'underline' }}>войти в систему</Link></>
                  }
                </span>
              </div>
            )}
